...
/*
 * THIS FILE WAS ORIGINALLY GENERATED ON Fri Nov 10 13:34:54 2017 EST
 * BASED ON THE FILE: e3xx_tx.xml
 * YOU *ARE* EXPECTED TO EDIT IT
 *
 * This file contains the implementation skeleton for the e3xx_tx worker in C++
 */

#include "e3xx_tx-worker.hh"
...
using namespace E3xx_txWorkerTypes;
namespace OA = OCPI::API;

class E3xx_txWorker : public E3xx_txWorkerBase {
...
public:
  ...
  E3xx_txWorker() : E3xx_txWorkerBase(),
      m_worker_str("e3xx_tx.rcc"),
      ...
      previousSmaPort("")
  {
    ...
    m_map_fixedval["rf_gain_max_dB"              ] = E3XX_TX_RF_GAIN_MAX_DB_P;
    m_map_fixedval["rf_gain_min_dB"              ] = E3XX_TX_RF_GAIN_MIN_DB_P;
    m_map_fixedval["rf_gain_step_dB"             ] = E3XX_TX_RF_GAIN_STEP_DB_P;

    m_map_fixedval[PROP_STR_BB_GAIN_DB           ] = E3XX_TX_NOT_SUPPORTED_P;
    m_map_fixedval["bb_gain_max_dB"              ] = E3XX_TX_BB_GAIN_MAX_DB_P;
    m_map_fixedval["bb_gain_min_dB"              ] = E3XX_TX_BB_GAIN_MIN_DB_P;
    m_map_fixedval["bb_gain_step_dB"             ] = E3XX_TX_BB_GAIN_STEP_DB_P;

    // frequency_MHz is not fixed
    m_map_fixedval["frequency_max_MHz"           ] = E3XX_TX_FREQUENCY_MAX_MHZ_P;
    m_map_fixedval["frequency_min_MHz"           ] = E3XX_TX_FREQUENCY_MIN_MHZ_P;
    // frequency_step_MHz is not fixed

    // sample_rate_MHz is not fixed
    m_map_fixedval["sample_rate_min_MHz"         ] = E3XX_TX_SAMPLE_RATE_MIN_MHZ_P;
    // sample_rate_step_MHz is not fixed

    m_map_fixedval[PROP_STR_RF_CUTOFF_FREQUENCY_MHZ] = E3XX_TX_NOT_SUPPORTED_P;
    m_map_fixedval["rf_cutoff_frequency_max_MHz" ] = E3XX_TX_RF_CUTOFF_FREQUENCY_MAX_MHZ_P;
    m_map_fixedval["rf_cutoff_frequency_min_MHz" ] = E3XX_TX_RF_CUTOFF_FREQUENCY_MIN_MHZ_P;
    m_map_fixedval["rf_cutoff_frequency_step_MHz"] = E3XX_TX_RF_CUTOFF_FREQUENCY_STEP_MHZ_P;

    // bb_cutoff_frequency_MHz is not fixed
    m_map_fixedval["bb_cutoff_frequency_max_MHz" ] = E3XX_TX_BB_CUTOFF_FREQUENCY_MAX_MHZ_P;
    m_map_fixedval["bb_cutoff_frequency_min_MHz" ] = E3XX_TX_BB_CUTOFF_FREQUENCY_MIN_MHZ_P;
...
private:
  ...
  const char* get_min_allowed_val(const std::string prop, double& min) {
    ...
    {
      min =   E3XX_TX_RF_GAIN_MIN_DB_P;
    }
    else if(prop == PROP_STR_BB_GAIN_DB)
    {
      min =   E3XX_TX_BB_GAIN_MIN_DB_P;
    }
    else if(prop == PROP_STR_FREQUENCY_MHZ)
    {
      min =   E3XX_TX_FREQUENCY_MIN_MHZ_P;
    }
    else if(prop == PROP_STR_SAMPLE_RATE_MHZ)
    {
      min =   E3XX_TX_SAMPLE_RATE_MIN_MHZ_P;
    }
    else if(prop == PROP_STR_RF_CUTOFF_FREQUENCY_MHZ)
    {
      min =   E3XX_TX_RF_CUTOFF_FREQUENCY_MIN_MHZ_P;
    }
    else if(prop == PROP_STR_BB_CUTOFF_FREQUENCY_MHZ)
    {
      min =   E3XX_TX_BB_CUTOFF_FREQUENCY_MIN_MHZ_P;
    }
    else
    {
  ...
  const char* get_max_allowed_val(const std::string prop, double& max) {
    if(prop == PROP_STR_RF_GAIN_DB)
    {
      max =   E3XX_TX_RF_GAIN_MAX_DB_P;
    }
    else if(prop == PROP_STR_BB_GAIN_DB)
    {
      max =   E3XX_TX_BB_GAIN_MAX_DB_P;
    }
    else if(prop == PROP_STR_FREQUENCY_MHZ)
    {
      max =   E3XX_TX_FREQUENCY_MAX_MHZ_P;
    }
    else if(prop == PROP_STR_SAMPLE_RATE_MHZ)
    {
      OCPI::API::Application& app = getApplication();
    ...
    {
      max =   E3XX_TX_RF_CUTOFF_FREQUENCY_MAX_MHZ_P;
    }
    else if(prop == PROP_STR_BB_CUTOFF_FREQUENCY_MHZ)
    {
      max =   E3XX_TX_BB_CUTOFF_FREQUENCY_MAX_MHZ_P;
    }
    else
    {
  ...
  void get_adjusted_val_to_be_applied_to_hw(
    ...
    else if(prop.compare(PROP_STR_RF_GAIN_DB) == 0)
    {
      ...
      double desired_nsteps = val_desired / E3XX_TX_RF_GAIN_STEP_DB_P;
      double desired_nsteps_rounded = round(desired_nsteps);
      double x = desired_nsteps_rounded * E3XX_TX_RF_GAIN_STEP_DB_P;
  ...
  RCCResult set_in_situ_val(const std::string& prop,
    ...
    if(prop.compare(PROP_STR_FREQUENCY_MHZ) == 0)
    {
      ...
      OCPI::API::Property filter_freq(app, filter_proxy_inst, "tx_frequency_MHz");

      double val_MHz = HZ_TO_MHZ(val_adjusted);
      filter_freq.setDoubleValue(val_MHz);    // Set the proper analog filterbank/switch settings

      std::string currentSmaPort = (m_properties.config.SMA_channel == CONFIG_SMA_CHANNEL_TRXA ? "trxa_mode" : "trxb_mode");
      if (previousSmaPort != "" && previousSmaPort != currentSmaPort) {
          std::string current_mode = "";
          app.getProperty(filter_proxy_inst, previousSmaPort.c_str(), current_mode);
          if (current_mode == "tx") {
              app.setProperty(filter_proxy_inst, previousSmaPort.c_str(), "off");
          }
      }
      // Only update the SMA mode if its different from the previous setting
      if (previousSmaPort != currentSmaPort) {
          app.setProperty(filter_proxy_inst, currentSmaPort.c_str(), "tx");
          previousSmaPort = currentSmaPort;
      }

      const char* config_inst = m_properties.app_inst_name_ad9361_config_proxy;
      OCPI::API::Property tx_rf_port_output(app, config_inst, "tx_rf_port_output");
      // From Ettus' tables found at
      //   https://files.ettus.com/manual/page_usrp_e3x0.html#e3x0_dboard_e310_frontend_a_switches
      // TX port A is used for frequencies >= 2940, B is used otherwise
      // Get the string values for the TXA, TXB enums from the config proxy. Use those casted write vals.
      long int TX_A_or_B;
      if (m_properties.frequency_MHz >= 2940.0)
      {
        std::string TXA_str = get_TXA_str();
        TX_A_or_B = strtol(TXA_str.c_str(), NULL, 0);
      }
      else // freq < 2940.0
      {
        std::string TXB_str = get_TXB_str();
        TX_A_or_B = strtol(TXB_str.c_str(), NULL, 0);
      }
      tx_rf_port_output.setULongValue((uint32_t) (TX_A_or_B & 0xffffffff));
    }
    else
    {
  ...
  RCCResult do_written(const std::string& prop, bool& previously_skipped) {
    ...
    {
      log_debug("executing previously skipped %s property initialization because 'init' property has been initialized", prop.c_str());
  ...
  std::string get_TXA_str() {
    std::string TXA_str;
    OCPI::API::Application &app = getApplication();
    const char* inst = m_properties.app_inst_name_ad9361_config_proxy;

    app.getProperty(inst, "TXA", TXA_str);

    return TXA_str;
  }

  std::string get_TXB_str() {
    std::string TXB_str;
    OCPI::API::Application &app = getApplication();
    const char* inst = m_properties.app_inst_name_ad9361_config_proxy;

    app.getProperty(inst, "TXB", TXB_str);
  ...
  RCCResult bb_cutoff_frequency_step_MHz_read() {
    ...
    m_properties.bb_cutoff_frequency_step_MHz = E3XX_TX_NOT_SUPPORTED_P;
  ...
  RCCResult config_written() {
    ...
    else // i.e. (m_properties.config.SMA_channel == CONFIG_SMA_CHANNEL_TRXB)
    {
      ad9361_init.tx_channel_swap_enable = 0; // will be later applied
