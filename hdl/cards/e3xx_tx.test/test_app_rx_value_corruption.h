...
#define APP_RX_CORRUPTION_E3XX_XML "app_rx_corruption_e3xx.xml"
...
bool did_pass_test_ocpi_app_rx_corruption_rx_rf_gain_dB()
{
  ...
  try
  {
    OCPI::API::Application app(APP_RX_CORRUPTION_E3XX_XML, NULL);
    app.initialize();
    app.start();
...
bool did_pass_test_ocpi_app_rx_value_corruption()
{
  ...
  // bb_gain_dB is unused for the e3xx_rx.rcc worker
  ...
  // bb_cutoff_frequency_MHz is unused for the e3xx_rx.rcc worker
