  ...
  {
    {
      OCPI::API::Property p(app, APP_DEFAULT_XML_INST_NAME_RX, prop.c_str());
      double actual_rx_val = p.getDoubleValue();
      TEST_EXPECTED_VAL_DIFF(actual_rx_val, expected_rx_val, 0.000004769); // rounding up from 4.768 in e3xx_rx.xml
    }

    {
      ...
      TEST_EXPECTED_VAL_DIFF((double)actual_ad9361_config_proxy_val, (double)expected_ad9361_config_proxy_val, 4.769); // rounding up from 4.768 in e3xx.xml
    }
  }
  catch (std::string &e) {
    fprintf(stderr, "Exception thrown: %s\n", e.c_str());
  ...
  {
    {
      OCPI::API::Property p(app, APP_DEFAULT_XML_INST_NAME_TX, prop.c_str());
      double actual_tx_val = p.getDoubleValue();
      TEST_EXPECTED_VAL_DIFF(actual_tx_val, expected_tx_val, 0.000004769); // rounding up from 4.768 in e3xx_rx.xml
    }
