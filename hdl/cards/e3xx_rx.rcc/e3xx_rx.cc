 ...
 *
 * This file contains the implementation skeleton for the e3xx_rx worker in C++
 */
...
using namespace E3xx_rxWorkerTypes;
...
public:
  ...
  E3xx_rxWorker() : E3xx_rxWorkerBase(),
      m_worker_str("e3xx_rx.rcc"),
      ...
      previousSmaPort("")
  {
    ...
    m_map_fixedval["rf_gain_step_dB"             ] = E3XX_RX_RF_GAIN_STEP_DB_P;

    m_map_fixedval[PROP_STR_BB_GAIN_DB           ] = E3XX_RX_NOT_SUPPORTED_P;
    m_map_fixedval["bb_gain_max_dB"              ] = E3XX_RX_BB_GAIN_MAX_DB_P;
    m_map_fixedval["bb_gain_min_dB"              ] = E3XX_RX_BB_GAIN_MIN_DB_P;
    m_map_fixedval["bb_gain_step_dB"             ] = E3XX_RX_BB_GAIN_STEP_DB_P;

    // frequency_MHz is not fixed
    m_map_fixedval["frequency_max_MHz"           ] = E3XX_RX_FREQUENCY_MAX_MHZ_P;
    m_map_fixedval["frequency_min_MHz"           ] = E3XX_RX_FREQUENCY_MIN_MHZ_P;
    ...
    m_map_fixedval[PROP_STR_RF_CUTOFF_FREQUENCY_MHZ] = E3XX_RX_NOT_SUPPORTED_P;
    m_map_fixedval["rf_cutoff_frequency_max_MHz" ] = E3XX_RX_RF_CUTOFF_FREQUENCY_MAX_MHZ_P;
    m_map_fixedval["rf_cutoff_frequency_min_MHz" ] = E3XX_RX_RF_CUTOFF_FREQUENCY_MIN_MHZ_P;
    m_map_fixedval["rf_cutoff_frequency_step_MHz"] = E3XX_RX_RF_CUTOFF_FREQUENCY_STEP_MHZ_P;

    // bb_cutoff_frequency_MHz is not fixed
    m_map_fixedval["bb_cutoff_frequency_max_MHz" ] = E3XX_RX_BB_CUTOFF_FREQUENCY_MAX_MHZ_P;
    m_map_fixedval["bb_cutoff_frequency_min_MHz" ] = E3XX_RX_BB_CUTOFF_FREQUENCY_MIN_MHZ_P;
...
private:
  ...
  const char* get_min_allowed_val(const std::string prop, double& min) {
    ...
    {
      min =   E3XX_RX_BB_GAIN_MIN_DB_P;
    }
    else if(prop == PROP_STR_FREQUENCY_MHZ)
    {
      min =   E3XX_RX_FREQUENCY_MIN_MHZ_P;
    }
    else if(prop == PROP_STR_SAMPLE_RATE_MHZ)
    {
      min =   E3XX_RX_SAMPLE_RATE_MIN_MHZ_P;
    }
    else if(prop == PROP_STR_RF_CUTOFF_FREQUENCY_MHZ)
    {
      min =   E3XX_RX_RF_CUTOFF_FREQUENCY_MIN_MHZ_P;
    }
    else if(prop == PROP_STR_BB_CUTOFF_FREQUENCY_MHZ)
    {
      min =   E3XX_RX_BB_CUTOFF_FREQUENCY_MIN_MHZ_P;
    }
    else
    {
  ...
  const char* get_max_allowed_val(const std::string prop, double& max) {
    ...
    {
      max =   E3XX_RX_BB_GAIN_MAX_DB_P;
    }
    else if(prop == PROP_STR_FREQUENCY_MHZ)
    {
      max =   E3XX_RX_FREQUENCY_MAX_MHZ_P;
    }
    else if(prop == PROP_STR_SAMPLE_RATE_MHZ)
    {
      OCPI::API::Application& app = getApplication();
    ...
    {
      max =   E3XX_RX_RF_CUTOFF_FREQUENCY_MAX_MHZ_P;
    }
    else if(prop == PROP_STR_BB_CUTOFF_FREQUENCY_MHZ)
    {
      max =   E3XX_RX_BB_CUTOFF_FREQUENCY_MAX_MHZ_P;
    }
    else
    {
  ...
  RCCResult bb_cutoff_frequency_step_MHz_read() {
    ...
    // to -1 for now
    m_properties.bb_cutoff_frequency_step_MHz = E3XX_RX_NOT_SUPPORTED_P;
