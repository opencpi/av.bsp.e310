 ...
 *  @brief Provides functions for reading in-situ Receiver (RX) Radio
...
{
  std::string enum_str;
  app.getProperty(app_inst_name_proxy, "rx_vco_divider", enum_str);
  if((enum_str == "2")  ||
     (enum_str == "4")  ||
     (enum_str == "8")  ||
     (enum_str == "16") ||
    ...
    double& val)
{
  ...
  //log_debug("d_Rx_RFPLL_N_Integer=%.15f", d_Rx_RFPLL_N_Integer);
  //log_debug("d_Rx_RFPLL_N_Fractional=%.15f", d_Rx_RFPLL_N_Fractional);
