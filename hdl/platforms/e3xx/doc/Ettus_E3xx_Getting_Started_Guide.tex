...
\begin{verbatim}
$ cd /etc/udev/rules.d/
$ sudo ln -s /<install-path>/opencpi/cdk/e3xx/host-udev-rules/98-e3xx.rules \
98-e3xx.rules
\end{verbatim}
\item Whether installed via RPMs or source (and manually creating the symbolic link), the USB to Serial adapter will be connected as \texttt{/dev/e3xx\_0} with read and write permissions for all users.
\end{itemize}

\noindent Once the E310 is powered on, use the following command to connect to the serial port:
\begin{verbatim}
