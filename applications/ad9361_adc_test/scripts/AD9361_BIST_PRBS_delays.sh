...
run_delay_tests_1r1t() {
  ...
  do
    ...
    OCPI_LIBRARY_PATH=$OCPI_LIBRARY_PATH:./assemblies/ P="-pfile_write=filename=$FILENAME $@" RX_DATA_CLOCK_DELAY=$clkdelay RX_DATA_DELAY=$datadelay ./scripts/run_app.sh $APP_XML $APP_RUNTIME_SEC $twortwot > $LOGFILENAME 2>&1
