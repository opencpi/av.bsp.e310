...
int main(int argc, char **argv) {
  ...
  double runtime = 3;
  enum HdlPlatform           {matchstiq_z1, zed, alst4, alst4x, ml605, e3xx};
  enum HdlPlatformRFFrontend {matchstiq_z1_Frontend, zipperFrontend, FMCOMMS2Frontend, FMCOMMS3Frontend, e3xxFrontend};
  ...
  for (unsigned n = 0; (container = OA::ContainerManager::get(n)); n++)
  {
    ...
    else if (container->model() == "hdl" && container->platform() == "e3xx")
    {
      currentPlatform = e3xx;
      currentFrontend = e3xxFrontend;
      validContainerFound = true;
      printf("FSK App for E3XX\n");
    }
  ...
  try {
    ...
    else if(currentFrontend == e3xxFrontend)
    {
      if (mode == "rx")
      {
        xml_name = "app_fsk_rx_e3xx.xml";
      }
      else if (mode == "tx")
      {
        xml_name = "app_fsk_tx_e3xx.xml";
      }
      else if (mode == "txrx")
      {
        xml_name = "app_fsk_txrx_e3xx.xml";
      }
      else if (mode == "bbloopback")
      {
        usage(argv0,"Error: bbloopback mode not supported on e3xx frontends.\n");
        //! @todo TODO/FIXME - support digital AD9361 loopback (which is *not* "B"ase"B"and loopback)
        //xml_name = "app_fsk_txrx_fmcomms3.xml";
      }
      else if (mode == "filerw")
    ...
    if (mode == "rx" || mode == "tx" || mode == "txrx" || mode == "bbloopback")
    {
      switch (currentFrontend)
      {
        ...
        case FMCOMMS3Frontend:
        case e3xxFrontend: runtime = 20;
                           break;
    ...
    else if(currentFrontend == e3xxFrontend)
    {
      ...
      if ((mode == "rx") || (mode == "txrx"))
      {
        cout << "Enter FMCOMMS RX SMA channel(TRXA, RX2A, RX2B or TRXB)" << endl;
        std::getline(std::cin, input);
      ...
      if ((mode == "tx") || (mode == "txrx"))
      {
        cout << "Enter FMCOMMS TX SMA channel(TRXA or TRXB)" << endl;
        std::getline(std::cin, input);
        if (input != "TRXA" &&
          input != "TRXB") {
    ...
    if ((mode == "rx" || mode == "txrx") and
        ((currentFrontend == FMCOMMS2Frontend) or
         (currentFrontend == FMCOMMS3Frontend) or
         (currentFrontend == e3xxFrontend)))
    {
      // set known-good defaults for FMCOMMS2/3 and e3xx
      rx_sample_rate = 4.;
      ...
      { // (currentFrontend == e3xxFrontend)
        // set known-good defaults for e3xx
        rx_rf_gain = 12.;
      }
