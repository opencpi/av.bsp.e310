...
const HdlPlatformRFFrontend defaultFrontend = e3xxFrontend;
...
static void usage(const char *name, const char *error_message) {
  fprintf(stderr,
    ...
    "    sma_channel        # specify which PCB SMA is used\n"
         "Example: rx_app 2400 2.5 -1 24 2.5 -1 0.1 1 1 e3xx TRXA\n",
                 error_message,name);
  exit(1);
}
...
int main(int argc, char **argv) {
  ...
  for (unsigned n = 0; (container = OA::ContainerManager::get(n)); n++)
  {
    ...
    else if (container->platform() == "e3xx")
    {
      currentFrontend = e3xxFrontend;
      if(argc == 12) {
        sma_channel = std::string(argv[11]);
